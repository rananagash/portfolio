...
const MiniPortrait = styled.div`display:flex;align-items:center;gap:12px;margin-top:28px;width:max-content;padding:7px 13px 7px 7px;background:${theme.colors.white};border:1.5px solid ${theme.colors.line};border-radius:999px;box-shadow:3px 4px 0 rgba(24,24,23,.14);transform:rotate(-2deg);img{width:58px;height:58px;border-radius:50%;object-fit:cover;object-position:50% 58%;border:1.5px solid ${theme.colors.line}}span{font:800 .7rem/1.35 ${theme.fonts.mono};text-transform:uppercase;letter-spacing:.04em}`;
...
const Bubble = styled.div`position:absolute;right:0;top:8%;max-width:235px;padding:12px 14px;background:${theme.colors.white};border:1.5px solid ${theme.colors.line};border-radius:13px 13px 13px 3px;box-shadow:4px 5px 0 rgba(0,0,0,.12);font:700 clamp(.65rem,1.2vw,.78rem)/1.4 ${theme.fonts.mono};span{color:${theme.colors.blue};}`;
...
const Hero = () => {
  ...
  return <HeroSection id="top"><div className="container"><Grid>
    <motion.div initial={reduce?false:{opacity:0,y:22}} animate={{opacity:1,y:0}} transition={{duration:.65}}>
      ...
      <Actions><Button href="#work">Explore my work <span>↓</span></Button><Button $secondary href="mailto:nagashrana@gmail.com">Say hello ↗</Button></Actions>
      <MiniPortrait><img src="/images/me.jpeg" alt="Rana Nagash"/><span>Builder behind<br/>the pixels</span></MiniPortrait>
    </motion.div>
